...
int DF_SetPlayerState(centity_t *cent)
{
	...
	if (cg.predictedPlayerState.m_iVehicleNum) {
		...
		state.velocity = (cent->currentState.clientNum == cg.clientNum ? vehCent->playerState->velocity : vehCent->currentState.pos.trDelta);
	}
	else {
		state.velocity = (cent->currentState.clientNum == cg.clientNum ? cg.predictedPlayerState.velocity : cent->currentState.pos.trDelta);
	}
...
void DF_DrawPitchHud ( float pitch ) {
	...
	color[0] = atof(COM_Parse(&t));
	color[1] = atof(COM_Parse(&t));
	color[2] = atof(COM_Parse(&t));
	color[3] = atof(COM_Parse(&t));
